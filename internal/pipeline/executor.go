...
// resolveCommandConfig constructs a CommandConfig by interpolating the stage's
// command fields against the provided runtime context.
//
// It returns the populated CommandConfig, or an error if interpolation of the
// Program, Cwd, any argument, or any environment value fails. Errors are wrapped
// with the stage ID and the specific field that failed (e.g., "program",
// "cwd", "arg[index]", or "env.key").
...
// overlays any entries from stageEnv. Keys are sorted for stable ordering.
...
// prefixing with "FABRIC_PIPELINE_ARTIFACT_".
