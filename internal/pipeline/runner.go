...
// NewRunner creates a Runner that writes to the provided stdout and stderr and uses the given plugin registry.
...
// If Input.From has an unsupported value, an error is returned.
...
// buildArtifactMap builds a map from artifact names to their file-system paths under the given run directory.
// The map keys are artifact names and the values are the corresponding paths obtained by joining runDir with each artifact's configured path.
...
// resolveStageOutputs determines artifact paths produced by a stage, collects the set of files
// written by the stage, and resolves the stage's primary payload.
//
// For each declared artifact, the returned map maps artifact name -> absolute filesystem path.
// The returned slice contains deduplicated paths of files written by the stage (including any
// files the executor reported and artifacts that exist or were created). The primary payload is
// the stage's stdout by default, or the contents of a designated primary artifact when
// PrimaryOutput.From is set to PrimaryOutputArtifact.
//
// The function will create and write the primary artifact from stdout when the primary output
// targets an artifact that does not exist but stdout is non-empty. It returns an error if a
// required artifact is missing or if filesystem operations (stat/read/write) fail.
...
// displayPathsForRun produces user-displayable paths for a run by attempting to relativize
// each non-empty path to runDir when possible; paths that would traverse outside runDir
// (start with "..") or cannot be relativized are returned unchanged. The result preserves
...
// with duplicates removed while preserving the first-seen order.
...
// validateAcceptedSource checks whether the pipeline accepts the given source mode.
// It returns nil if the pipeline's Accepts list is empty or contains the mode.
// Otherwise it returns an error indicating the pipeline does not accept the provided mode.
...
// SelectStageIndices determines which stage indices should be executed based on the
// provided from/to/only stage filters. It accepts a slice of stages and at most one
// of the range filters (--from-stage/--to-stage) or the single-stage filter (--only-stage).
//
// If onlyStage is provided it returns that single stage's index. If fromStage or toStage
// are provided they set the inclusive start and end of the selected range.
// Errors are returned for an empty stage list, an invalid combination of flags
// (onlyStage with from/to), unknown stage IDs, or when fromStage appears after toStage.
//
// It returns a slice of selected indices in ascending order and a set (map[int]struct{})
// containing those indices for fast membership checks.
...
// stageIDsFromIndices returns the stage IDs corresponding to the provided indices.
// The function indexes into the stages slice using each index in indices; callers must ensure indices are within bounds.
...
// findLastValidateStageIndexForSelection returns the last index among the selected stages whose role is Validate, or -1 if no selected stage has the Validate role.
...
// persistRunManifest writes the RunManifest as indented JSON to runDir/run_manifest.json.
// It returns any error encountered while marshaling or writing the file.
...
// persistRunState writes the run manifest and the run state JSON files into the specified run directory.
// It writes run_manifest.json followed by run.json and returns any error encountered while persisting them.
...
// writePublishManifests writes per-stage publish manifest JSON files and updates
// the provided RunManifest's stage file lists with displayable paths for the run.
// It groups requested publish manifest paths by stage, prepends those paths to
// each stage's Files list (after converting to displayable paths relative to
// runDir), ensures target directories exist, and writes a JSON payload for each
// request containing the run manifest, the source (mode and reference), and the
// final output. Returns an error if directory creation or file writing fails.
...
// writeJSON writes value as indented JSON to the file at path using 0644 permissions.
// It marshals value with two-space indentation and writes the resulting bytes to path.
// The returned error is annotated with the operation and path when marshaling or writing fails.
