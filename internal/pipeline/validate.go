...
// Validate reports whether p is a well-formed Pipeline and returns an error for the first validation failure.
// It returns an error describing the first validation failure, or nil if the pipeline is valid.
//
// Validation performed includes: non-nil pipeline, version >= 1, non-empty name and optional file-stem consistency,
// at least one stage, supported source modes in Accepts, unique stage IDs, valid stage roles and executor configuration,
// artifact declarations (names, unique per stage, relative paths that do not escape the run directory),
// stage input and primary output correctness, exactly one final_output stage with a primary_output,
// role ordering constraints (validate cannot appear after publish; validate/publish stages must appear after final_output),
// and cross-stage artifact reference resolution.
...
// stageDeclaresArtifact reports whether stage declares an artifact with the given name.
// It returns true if any artifact in stage.Artifacts has Name equal to name, false otherwise.
