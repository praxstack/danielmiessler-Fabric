...
func TestNoteEnhancementPipelineLoadsAndValidates(t *testing.T) {
	...
	pipe, err := loader.LoadNamed("enhance-notes")
	require.NoError(t, err)
...
func TestNoteEnhancementPrepareSourceStdinUsesRunDirArtifacts(t *testing.T) {
	...
	prepareStdout := runPythonScript(
		t,
		repoRoot,
		filepath.Join(repoRoot, "scripts", "pipelines", "enhance-notes", "prepare_source.py"),
		nil,
...
func TestNoteEnhancementMaterializeRejectsDuplicateArtifactBlocks(t *testing.T) {
	...
	runPythonScript(
		t,
		fixture.RepoRoot,
		filepath.Join(fixture.RepoRoot, "scripts", "pipelines", "enhance-notes", "prepare_source.py"),
		nil,
		"",
		map[string]string{
			"FABRIC_PIPELINE_RUN_DIR":          runDir,
	...
	_, stderr := runPythonScriptExpectError(
		t,
		fixture.RepoRoot,
		filepath.Join(fixture.RepoRoot, "scripts", "pipelines", "enhance-notes", "materialize_enhanced_notes.py"),
...
func TestNoteEnhancementValidateRequiresTitleAtDocumentStart(t *testing.T) {
	...
	_, stderr := runPythonScriptExpectError(
		t,
		fixture.RepoRoot,
		filepath.Join(fixture.RepoRoot, "scripts", "pipelines", "enhance-notes", "run_validate_stage.py"),
		[]string{"--session-context", sessionContextPath},
		"",
		map[string]string{
			"FABRIC_PIPELINE_RUN_DIR": runDir,
