#!/usr/bin/env python3
"""Shared helpers for the Fabric-native nontechnical-study-guide pipeline."""
...
def resolve_input_path(path: Path) -> Path:
    ...
    if resolved.is_dir():
        ...
        if len(candidates) == 1:
            return candidates[0]
        raise FileNotFoundError(f"Could not resolve unique nontechnical-study-guide input in directory: {resolved}")
...
def resolve_session_context(stdin_text: str) -> tuple[SessionContext, str]:
    ...
    if current_source_mode == "source" and current_source_reference:
        ...
        raise RuntimeError(
            "nontechnical-study-guide supports source files/directories, stdin, or scrape_url input; "
